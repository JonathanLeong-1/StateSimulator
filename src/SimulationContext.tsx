...
function buildWorld(seed: number, settings: SimSettings): { world: WorldData; engine: SimulationEngine } {
  ...
  const world = generator.generate({ width: DEFAULT_GRID.width, height: DEFAULT_GRID.height, seed, seaConquestRadius: 4 });
  const engine = new SimulationEngine(world, settings);
  engine.initialize();
  return { world, engine };
}

function buildCircleWorld(
...
export function SimulationProvider({ children }: { children: ReactNode }) {
  ...
  const randomizeContinents = useCallback(() => {
    const WIDTH = DEFAULT_GRID.width, HEIGHT = DEFAULT_GRID.height;
    ...
    for (let b = 0; b < numBlobs; b++) {
      blobCenters.push({ q: Math.floor(rng() * WIDTH), r: Math.floor(rng() * HEIGHT), radius: minDim * 0.08 + rng() * minDim * 0.12 });
    }
    const tiles: MapBuilderTile[] = [];
    for (let i = 0; i < WIDTH * HEIGHT; i++) {
